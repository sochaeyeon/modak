...
public class BoardService {
	...
	public List<Map<String, Object>> getRecentPosts(String targetUserId, int limit) {
	    HashMap<String, Object> map = new HashMap<>();
	    map.put("userId", targetUserId);
	    map.put("limit", limit);
	    return mapper.selectRecentBoardsByUser(map);
	}
	...
	@Transactional
	public HashMap<String, Object> writeBoard(HashMap<String, Object> map, MultipartFile[] files, String question,
			List<String> options, String endDate, List<String> tags, // ★ 추가
		...
		try {
			...
			if (parentId == null || "".equals(String.valueOf(parentId).trim())) {
				...
				if (boardInfo != null) {
					...
					if (!receiverId.equals(senderId)) {
						alarmService.createAlarm(receiverId, "BOARD_COMMENT", "게시글에 댓글이 달렸어요",
								"작성한 게시글에 새로운 댓글이 등록되었습니다.", map.get("boardId"));
					}
				}
			}
			...
			if (parentId != null && !"".equals(String.valueOf(parentId).trim())) {
				...
				if (parentComment != null) {
					...
					if (!receiverId.equals(senderId)) {
						alarmService.createAlarm(receiverId, "BOARD_REPLY", "내 댓글에 답글이 달렸어요", "작성한 댓글에 새 답글이 등록되었습니다.",
								map.get("boardId"));
					}
				}
			}
	...
	public HashMap<String, Object> toggleReaction(HashMap<String, Object> map) {
		...
		try {
			...
			if (isBlank(map.get("commentId")))
				map.put("commentId", null);
			if (isBlank(map.get("boardId")))
			...
			if (exists > 0) {
				...
				if (isBoard) {
					if ("LIKE".equals(type))
						mapper.decreaseBoardLikeCount(map);
					else
						mapper.decreaseBoardDislikeCount(map);
				} else {
					...
					if (isBoard) {
						...
						if (boardInfo != null) {
							...
							if (!receiverId.equals(userId)) {
								alarmService.createAlarm(receiverId, "BOARD_LIKE", "게시글에 추천이 눌렸어요",
										"작성한 게시글에 추천이 추가되었습니다.", map.get("boardId"));
							}
						}
					} else {
						...
						if (commentInfo != null) {
							...
							if (!receiverId.equals(userId)) {
								alarmService.createAlarm(receiverId, "COMMENT_LIKE", "댓글에 좋아요가 눌렸어요",
										"작성한 댓글에 좋아요가 추가되었습니다.", commentInfo.get("BOARD_ID"));
							}
						}
					}
				}
				if (isBoard) {
					if ("LIKE".equals(type))
						mapper.increaseBoardLikeCount(map);
					else
						mapper.increaseBoardDislikeCount(map);
				} else {
	...
	private void applyEditorThumbnail(List<Map<String, Object>> list) {
		...
		for (Map<String, Object> item : list) {
			...
			String editorImgUrl = extractFirstImgUrl(contentObj != null ? String.valueOf(contentObj) : null);
	...
	@Transactional
	public HashMap<String, Object> editBoard(HashMap<String, Object> map, String question, List<String> options,
			String endDate, List<String> tags) {
		...
		try {
			...
			boolean hasPoll = question != null && !question.isBlank() && options != null
