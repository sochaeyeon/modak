...
public class OrderExchangeController {
    ...
    @ResponseBody
    public String getExchangeInfo(@RequestParam HashMap<String, Object> map,
                                   HttpSession session) {
        String userId = (String) session.getAttribute("sessionId");
        if (userId == null) return "{\"result\":\"fail\",\"message\":\"로그인이 필요합니다.\"}";
    ...
    @ResponseBody
    public String applyExchange(@RequestParam HashMap<String, Object> map,
                                 HttpSession session) {
        String userId = (String) session.getAttribute("sessionId");
        if (userId == null) return "{\"result\":\"fail\",\"message\":\"로그인이 필요합니다.\"}";
        map.put("userId", userId);
