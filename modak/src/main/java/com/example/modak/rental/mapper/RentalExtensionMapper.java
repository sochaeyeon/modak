...
public interface RentalExtensionMapper {
	...
	/** 회원 단건 조회 */
	RentalExtension selectRentalByIdAndUser(@Param("rentalId") Long rentalId, @Param("userId") String userId);

	/** 비회원 3중 검증 조회 */
	RentalExtension selectGuestRental(@Param("rentalId") Long rentalId, @Param("guestName") String guestName,
	...
	// ════════════════════════════════════════
	// 연체료 결제 주문 (RENTAL_OVERDUE_ORDER)
	// ════════════════════════════════════════

	/** 연체료 결제 주문 INSERT */
	int insertOverdueOrder(HashMap<String, Object> map);

	/** 연체료 결제 주문 단건 조회 */
	HashMap<String, Object> selectOverdueOrder(HashMap<String, Object> map);

	/** 연체료 결제 주문 상태 업데이트 */
	int updateOverdueOrderStatus(HashMap<String, Object> map);

	/** 반납 신청 + 연체료 저장 */
	int updateReturnRequestWithOverdue(HashMap<String, Object> map);

	/** 비회원 반납 신청 + 연체료 저장 */
	int updateGuestReturnRequestWithOverdue(HashMap<String, Object> map);
	
	String selectLatestPaidOverdueDate(HashMap<String, Object> map);

	/** 특정 연체 단계(N일차)에 도달했고 아직 해당 단계 알림 안 보낸 건 조회 */
	List<HashMap<String, Object>> selectOverdueRentalsByStage(@Param("days") int days);

	/** 알림 발송 단계 업데이트 */
	int updateOverdueNotifyStage(@Param("rentalId") Long rentalId, @Param("stage") int stage);
}