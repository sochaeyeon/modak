        ...
        <div class="order-page-container">
            ...
            <div class="o-tab-bar">
                <button class="o-tab-btn" :class="{ active: activeTab === 'orders' }"
                    @click="switchTab('orders')">🧾 주문 내역</button>
                ...
                <table class="order-table">
                    ...
                    <tbody>
                        <tr v-if="orderList.length === 0">
                            <td colspan="6" style="padding:40px;color:var(--o-text-muted);">주문 내역이 없습니다.</td>
                        </tr>
            ...
            <div v-if="activeTab === 'delivery'" class="order-card" style="padding:28px;">
                <div style="font-size:15px;font-weight:700;color:#fff;margin-bottom:24px;">🚚 운송장 번호 등록</div>
                <div style="display:grid;grid-template-columns:1fr 1fr auto;gap:12px;align-items:end;max-width:700px;">
                    <div>
                        <label style="font-size:11px;color:var(--o-text-muted);display:block;margin-bottom:6px;">주문번호</label>
                        <input class="o-input" v-model="delivery.orderId" placeholder="주문번호 입력" type="text">
                    </div>
                    <div>
                        <label style="font-size:11px;color:var(--o-text-muted);display:block;margin-bottom:6px;">운송장 번호</label>
                ...
                <div style="margin-top:32px;">
                    <div style="font-size:13px;color:var(--o-text-muted);margin-bottom:12px;">최근 배송 등록 내역</div>
                    <table class="order-table">
                        <thead>
                            <tr>
                                <th>주문번호</th><th>주문자</th>
                                <th style="text-align:left;">상품명</th>
                                <th>운송장번호</th><th>출고일시</th><th>배송상태</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-if="deliveryList.length === 0">
                                <td colspan="6" style="padding:40px;color:var(--o-text-muted);">등록된 배송 내역이 없습니다.</td>
                            </tr>
                            <tr v-for="d in deliveryList" :key="d.DELIVERY_ID">
                                <td class="order-id">#{{ d.ORDER_ID }}</td>
                                <td>{{ d.USER_ID }}</td>
                                <td class="prod-info-td"><div>{{ d.PRODUCT_NAME }}</div></td>
                                <td style="font-family:monospace;color:var(--o-accent);">{{ d.TRACKING_NO }}</td>
            ...
            <div v-if="activeTab === 'returns'" class="order-card">
                <div class="return-filter-bar">
                    ...
                    <button class="o-filter-btn" :class="{ active: returnFilter === 'RETURN_REQUESTED' }"
                    ...
                    <button class="o-filter-btn" :class="{ active: returnFilter === 'RETURN_COMPLETED' }"
                ...
                <table class="order-table">
                    ...
                    <tbody>
                        <tr v-if="filteredReturnList.length === 0">
                            <td colspan="7" style="padding:40px;color:var(--o-text-muted);">반납 요청 내역이 없습니다.</td>
                        </tr>
                        <tr v-for="item in filteredReturnList" :key="item.RENTAL_ID">
                            <td class="order-id">#{{ item.RENTAL_ID }}</td>
                            <td>
                                <div style="font-weight:600;color:#fff;">{{ item.USER_ID === 'GUEST' ? item.GUEST_NAME : item.USER_ID }}</div>
                                <div style="font-size:11px;color:var(--o-text-muted);" v-if="item.USER_ID === 'GUEST'">{{ item.GUEST_PHONE }}</div>
                                <div style="font-size:10px;color:var(--o-accent);margin-top:2px;" v-if="item.USER_ID === 'GUEST'">비회원</div>
                            </td>
                            <td class="prod-info-td">
                                <div class="order-img-box">
                                ...
                                <div>
                                    ...
                                    <div style="font-size:11px;color:var(--o-text-muted);margin-top:2px;">아이템ID: {{ item.ITEM_ID }}</div>
                                </div>
                            </td>
                            ...
                            <td>
                                <div v-if="item.RETURN_ADDRESS" style="font-size:12px;line-height:1.5;">
                                    <div style="color:var(--o-text-muted);font-size:10px;">[{{ item.RETURN_ZIPCODE }}]</div>
                                    <div>{{ item.RETURN_ADDRESS }}</div>
                                    <div style="color:var(--o-text-muted);">{{ item.RETURN_DETAILED_ADDRESS }}</div>
                                </div>
                                <div v-else style="color:var(--o-text-muted);font-size:12px;">주소 없음</div>
                            </td>
                            <td style="font-size:12px;color:var(--o-text-muted);">{{ item.RETURN_REQUESTED_AT }}</td>
                            <td>
                                <div style="display:flex;flex-direction:column;gap:6px;">
                                    ...
                                    <button v-if="item.RENTAL_STATUS === 'RETURN_REQUESTED'"
                                        ...
                                        @click="fnUpdateReturnStatus(item, 'RETURN_PICKED')">🚚 수거 시작</button>
                                    <button v-if="item.RENTAL_STATUS === 'RETURN_PICKED'"
                                        class="o-action-btn complete"
                                        @click="fnUpdateReturnStatus(item, 'RETURN_COMPLETED')">✅ 반납 완료</button>
                                    <button v-if="item.RENTAL_STATUS === 'RETURN_COMPLETED'"
                                        class="o-action-btn cancel"
                                        @click="fnUpdateReturnStatus(item, 'RETURN_PICKED')">↩ 수거중으로</button>
                                </div>
                            </td>
                        </tr>
                    </tbody>
                ...
                <table class="order-table">
                    <thead>
                        <tr>
                            <th>대여번호</th><th>고객</th>
                            <th style="text-align:left">상품</th>
                            <th>반납일</th><th>검수상태</th><th>공제금액</th><th>메모</th><th>검수</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-if="inspectionList.length === 0">
                            <td colspan="8" style="padding:40px;color:var(--o-text-muted)">반납완료 내역이 없습니다.</td>
                        </tr>
                        <tr v-for="item in inspectionList" :key="item.RENTAL_ID">
                            ...
                            <td class="prod-info-td">
                                <div class="order-img-box">
                                    <img v-if="item.IMG_URL" :src="item.IMG_URL">
                                    <div v-else style="font-size:12px;display:flex;align-items:center;justify-content:center;height:100%">🏕️</div>
                                </div>
                                <div>{{ item.PRODUCT_NAME }}</div>
                            </td>
                            <td style="font-size:12px;color:var(--o-text-muted)">{{ item.RETURN_DATE }}</td>
                            <td>
                                <span v-if="item.CONDITION_CODE === 'GOOD'"    style="color:#58d68d;font-weight:700">✅ 양호</span>
                                <span v-else-if="item.CONDITION_CODE === 'DAMAGED'" style="color:#f39c12;font-weight:700">⚠ 파손</span>
                                <span v-else-if="item.CONDITION_CODE === 'LOST'"    style="color:#e74c3c;font-weight:700">❌ 분실</span>
                                <span v-else style="color:var(--o-text-muted)">미검수</span>
                            </td>
                            <td>{{ item.DEDUCTION_AMT ? Number(item.DEDUCTION_AMT).toLocaleString() + '원' : '-' }}</td>
                            <td style="font-size:12px;color:var(--o-text-muted);max-width:150px;">{{ item.MEMO || '-' }}</td>
                            <td>
            ...
            <div v-if="showInspectionModal"
                ...
                <div style="background:#1e2130;border-radius:16px;padding:32px;width:440px;box-shadow:0 8px 32px rgba(0,0,0,.4)">
                    <div style="font-size:16px;font-weight:700;color:#fff;margin-bottom:20px">🔍 검수 등록</div>
                    <div style="margin-bottom:14px">
                        <label style="font-size:12px;color:var(--o-text-muted);display:block;margin-bottom:6px">상품 상태</label>
                    ...
                    <div style="margin-bottom:14px" v-if="inspectionForm.conditionCode !== 'GOOD'">
                        <label style="font-size:12px;color:var(--o-text-muted);display:block;margin-bottom:6px">공제 금액 (원)</label>
                        <input class="o-input" type="number" v-model.number="inspectionForm.deductionAmt" style="width:100%">
                    </div>
                    <div style="margin-bottom:20px">
                        <label style="font-size:12px;color:var(--o-text-muted);display:block;margin-bottom:6px">메모</label>
            ...
            <div v-if="activeTab === 'refunds'" class="order-card">
                <table class="order-table">
                    <thead>
                        <tr>
                            <th>환불번호</th><th>주문번호</th><th>고객</th>
                            <th style="text-align:left">상품</th>
                            <th>환불금액</th><th>사유</th><th>상태</th><th>신청일</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-if="refundList.length === 0">
                            <td colspan="8" style="padding:40px;color:var(--o-text-muted)">환불 내역이 없습니다.</td>
                        </tr>
                        <tr v-for="item in refundList" :key="item.REFUND_ID">
                            ...
                            <td style="color:var(--o-accent);font-weight:700">{{ Number(item.REFUND_AMOUNT||0).toLocaleString() }}원</td>
                            <td style="font-size:12px;color:var(--o-text-muted)">{{ item.REFUND_REASON }}</td>
                            <td>
                                <span :style="{color: item.REFUND_STATUS==='COMPLETED'?'#58d68d': item.REFUND_STATUS==='PENDING'?'#f39c12':'#8a8f9d', fontWeight:'700', fontSize:'12px'}">
                                    {{ item.REFUND_STATUS==='COMPLETED' ? '✅ 완료' : item.REFUND_STATUS==='PENDING' ? '⏳ 처리중' : '❓' + item.REFUND_STATUS }}
                                </span>
                            </td>
                            <td style="font-size:12px;color:var(--o-text-muted)">{{ item.CREATED_AT }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div v-if="activeTab === 'exchanges'" class="order-card">
                <table class="order-table">
                    <thead>
                        <tr>
                            <th>교환번호</th><th>주문번호</th><th>고객</th>
                            <th style="text-align:left">상품</th>
                            <th>사유</th><th>회수주소</th><th>상태</th><th>신청일</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-if="exchangeList.length === 0">
                            <td colspan="8" style="padding:40px;color:var(--o-text-muted)">교환 내역이 없습니다.</td>
                        </tr>
                        <tr v-for="item in exchangeList" :key="item.EXCHANGE_ID">
                            ...
                            <td style="font-size:12px;color:var(--o-text-muted)">{{ item.REASON }}</td>
                            <td style="font-size:12px;color:var(--o-text-muted)">{{ item.ADDRESS }} {{ item.DETAILED_ADDRESS }}</td>
                            <td>
                                <div style="display:flex;flex-direction:column;gap:6px">
                                    <span :style="{color: item.STATUS==='APPROVED'?'#58d68d': item.STATUS==='REJECTED'?'#e74c3c': item.STATUS==='COMPLETED'?'#3498db':'#f39c12', fontWeight:'700', fontSize:'12px'}">
                                        {{ {REQUESTED:'📋 신청', APPROVED:'✅ 승인', REJECTED:'❌ 거절', COMPLETED:'🏁 완료'}[item.STATUS] || item.STATUS }}
                                    </span>
                                    <button v-if="item.STATUS==='REQUESTED'" class="o-action-btn complete"
                                        style="font-size:11px;padding:4px 8px"
                                        @click="fnUpdateExchangeStatus(item,'APPROVED')">승인</button>
                                    <button v-if="item.STATUS==='REQUESTED'" class="o-action-btn cancel"
                                        style="font-size:11px;padding:4px 8px"
                                        @click="fnUpdateExchangeStatus(item,'REJECTED')">거절</button>
                                    <button v-if="item.STATUS==='APPROVED'" class="o-action-btn pickup"
                                        style="font-size:11px;padding:4px 8px"
            ...
            methods: {
                ...
                fnOpenInspection(item) {
                    this.inspectionForm = {
                        ...
                        userId:        item.USER_ID,
                        conditionCode: item.CONDITION_CODE || 'GOOD',
                        deductionAmt:  item.DEDUCTION_AMT  || 0,
                        memo:          item.MEMO            || ''
                    };
                ...
                fnUpdateExchangeStatus(item, status) {
                    ...
                    $.ajax({
                        ...
                        data: { exchangeId: item.EXCHANGE_ID, userId: item.USER_ID, status: status },
                        success: (res) => {
                            if (res.result === 'success') {
                                item.STATUS = status;
                            } else {
