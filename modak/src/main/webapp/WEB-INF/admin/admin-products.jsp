                ...
                <%@ include file="/WEB-INF/admin/admin-sidebar.jsp" %>
                    ...
                    <div id="app" class="admin-main">
                        ...
                        <div class="p-modal-overlay" :class="{open: modalOpen}" @click.self="modalOpen=false">
                            <div class="p-modal">
                                <div
                                    style="font-size:20px; font-weight:700; margin-bottom:25px; color:#fff; display:flex; justify-content:space-between;">
                                ...
                                <textarea class="p-input" v-model="form.description" placeholder="상세 설명"
                                ...
                                <div style="display:grid; grid-template-columns: 1fr 2fr; gap:10px">
                                    ...
                                    <!-- featureContent 입력 아래, 버튼 행 위에 추가 -->
                                    ...
                                    <div class="stock-section">
                                        ...
                                        <div v-for="(stock, idx) in stockList" :key="idx" class="stock-row">
                                            <div style="display:flex;align-items:center;gap:8px;flex:1;">
                                                <span class="p-label" style="margin:0;white-space:nowrap;">
                                                    옵션 {{ stock.optionId || '-' }}
                                                </span>
                                            </div>
                                        ...
                                        <div class="stock-add-row">
                                            ...
                                            <input class="p-input stock-input" type="number"
                                                v-model.number="newStock.optionId" placeholder="옵션 ID (없으면 빈칸)">
                        ...
                        createApp({
                            data() {
                                return {
                                    ...
                                    stockList: [],                              // ← 추가
                                    newStock: { totalQty: 0, optionId: '' },    // ← 추가
                                    form: {
                            ...
                            methods: {
                                ...
                                fnOpenEdit(p) {
                                    // 기존 코드...
                                    this.modalOpen = true;
                                    this.fnLoadStock(p.PRODUCT_ID);  // ← 추가
                                },

                                fnOpenAdd() {
                                    // 기존 코드...
                                    this.stockList = [];             // ← 추가
                                    this.newStock = { totalQty: 0, optionId: '' };  // ← 추가
                                    this.modalOpen = true;
                                },
                                ...
                                fnAddStock() {
                                    ...
                                    if (this.newStock.totalQty < 0) { alert('수량은 0 이상이어야 합니다.'); return; }
                                    ...
                                    $.ajax({
                                        ...
                                        data: {
                                            ...
                                            optionId: this.newStock.optionId || '',
                                            totalQty: this.newStock.totalQty,
                                            reservedQty: 0,
                                            availableQty: this.newStock.totalQty
                                        },
                                        success: (res) => {
                                            const data = typeof res === 'string' ? JSON.parse(res) : res;
                                            if (data.result === 'success') {
