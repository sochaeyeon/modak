    ...
    <div id="app" v-cloak>
        <div class="page-wrap">
            ...
            <div class="card" v-if="board">
                <div class="card-body">
                    ...
                    <!-- 카테고리/뱃지 -->
                    <div class="post-cats">
                        <span class="cat-tag" :class="'cat-' + board.CATEGORY">{{ fnCatLabel(board.CATEGORY) }}</span>
                        ...
                        <div class="post-menu-wrap">
                            <button class="more-btn" type="button" @click.stop="postMenuOpen = !postMenuOpen">
                                <i class="ri-more-2-fill"></i>
                            </button>
                            <div class="more-menu" v-if="postMenuOpen" @click.stop>
                                <template v-if="isMyPost">
                                    <button type="button" @click="postMenuOpen = false; fnGoEdit();">수정</button>
                                    <button type="button" class="danger" @click="postMenuOpen = false; fnDeletePost();">삭제</button>
                                </template>
                                <template v-else>
                                    <button type="button" class="danger" @click="postMenuOpen = false; showReportModal = true;">신고</button>
                                </template>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- 메타 -->
                    <div class="post-meta">
                        <div class="author-avatar" @click="fnShowProfile($event, board.USER_ID)" style="cursor:pointer;">
                            <img v-if="board.profileImg" :src="board.profileImg" alt="프로필">
                            <div v-else class="default-avatar"><i class="ri-user-smile-line"></i></div>
                        </div>
                        <span style="font-weight:700;cursor:pointer;" @click="fnShowProfile($event, board.USER_ID)">
                            {{ board.nickName }}
                        </span>
                        <span class="dot">·</span>
                    ...
                    <div class="tag-wrap" v-if="tagList.length > 0">
                        <span class="board-tag" v-for="tag in tagList" :key="tag" @click="fnTagSearch(tag)">#{{ tag }}</span>
                    </div>
                    ...
                    <!-- 제품 링크 -->
                    <div class="product-link" v-if="board.productName" @click="fnGoProduct(board.PRODUCT_ID)">
                        ...
                        <div>
                            ...
                            <div style="font-size:14px;font-weight:700;color:var(--brown);">{{ board.productName }}</div>
                            <div style="font-size:12px;color:var(--brown3);">{{ Number(board.productPrice||0).toLocaleString() }}원</div>
                        </div>
                    </div>
                        ...
                        <div v-for="opt in pollOptions" :key="opt.optionId" class="poll-option"
                            ...
                            <div class="poll-option-inner">
                                <span class="poll-option-text">
                                    {{ opt.optionText }}
                                    <span v-if="poll.myOptionId == opt.optionId" style="color:var(--purple);">✔</span>
                                </span>
                    ...
                    <div class="reaction-bar">
                        <button class="react-btn" :class="{ liked: myLiked }" @click="fnReact('LIKE', 'BOARD')">
                        ...
                        <button class="react-btn" :class="{ disliked: myDisliked }" @click="fnReact('DISLIKE', 'BOARD')">
                    ...
                    <template v-for="comment in topComments" :key="comment.COMMENT_ID">
                        ...
                        <div class="comment-item">
                            <div class="comment-meta">
                                <div class="comment-avatar" @click="fnShowProfile($event, comment.USER_ID)" style="cursor:pointer;">
                            ...
                            <div class="comment-actions">
                                ...
                                <button class="reply-btn"
                                    ...
                                    <i :class="openReplies[comment.COMMENT_ID] ? 'ri-arrow-up-s-line' : 'ri-arrow-down-s-line'"></i>
                                </button>
                                <div class="comment-menu-wrap">
                                    ...
                                    <div class="comment-more-menu" v-if="openCommentMenuId === comment.COMMENT_ID" @click.stop>
                                        <button type="button" v-if="!isMyComment(comment.USER_ID)" class="danger"
                            ...
                            <div class="inline-reply-box" v-if="replyTarget && replyTarget.COMMENT_ID === comment.COMMENT_ID">
                                <textarea class="comment-textarea" v-model="commentContent"
                                    placeholder="답글을 입력하세요..." @keydown.ctrl.enter="fnSubmitComment"></textarea>
                                <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:8px;">
                                    <button class="reply-btn" @click="replyTarget = null; commentContent = ''">취소</button>
                                    <button class="btn-comment-submit small" @click="fnSubmitComment">답글 등록</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="reply-thread" v-if="openReplies[comment.COMMENT_ID]">
                            <div v-for="reply in getAllDescendants(comment.COMMENT_ID)" :key="reply.COMMENT_ID" class="comment-item reply">
                                <div class="comment-meta">
                                    <div class="comment-avatar" @click="fnShowProfile($event, reply.USER_ID)" style="cursor:pointer;">
                                ...
                                <div class="comment-content">
                                    <span class="reply-mention" v-if="getParentNick(reply)">@{{ getParentNick(reply) }}</span>
                                    {{ reply.CONTENT }}
                                </div>
                                <div class="comment-actions">
                                    <button class="comment-like-btn"
                                    ...
                                    <div class="comment-menu-wrap">
                                        ...
                                        <div class="comment-more-menu" v-if="openCommentMenuId === reply.COMMENT_ID" @click.stop>
                                            <button type="button" v-if="!isMyComment(reply.USER_ID)" class="danger"
                                ...
                                <div class="inline-reply-box" v-if="replyTarget && replyTarget.COMMENT_ID === reply.COMMENT_ID">
                                    <textarea class="comment-textarea" v-model="commentContent"
                                        placeholder="답글을 입력하세요..." @keydown.ctrl.enter="fnSubmitComment"></textarea>
                                    <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:8px;">
                                        <button class="reply-btn" @click="replyTarget = null; commentContent = ''">취소</button>
        ...
        <div v-if="profilePopup.show"
            ...
            <div style="margin-top:12px;display:flex;flex-direction:column;gap:7px;">
                <button @click="fnGoProfile(profilePopup.user.userId)"
                    style="width:100%;height:34px;border:1.5px solid var(--border);border-radius:10px;
                ...
                <button v-if="profilePopup.user.userId !== currentUserId"
                    class="mp-chat-btn"
                    :class="profilePopup.chatBtnState"
                    :disabled="profilePopup.chatBtnState === 'pending'"
                    @click="fnRequestChat(profilePopup.user.userId)">
                    <i :class="profilePopup.chatBtnState === 'exists' ? 'ri-chat-3-line' : 'ri-chat-new-line'"></i>
    ...
    <script>
        ...
        createApp({
            ...
            methods: {
                    ...
                    $.ajax({
                        ...
                        success: (res) => {
                            if (res.result === 'success') {
                                ...
                                this.showToast('⚠️ ' + (res.message || '신청할 수 없습니다.'));
                            }
                        },
                        error: () => {
                            pp.chatBtnLabel = '대화 신청';
                ...
                // ── 기존 메서드들 (변경 없음) ─────────────────
                fnToggleReplies(commentId) { this.openReplies[commentId] = !this.openReplies[commentId]; },
