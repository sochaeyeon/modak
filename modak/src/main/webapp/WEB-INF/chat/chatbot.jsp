    ...
    <body>
        ...
        <div id="app" v-cloak>
            <div class="sidebar">
                ...
                <p style="font-size:11px; color:var(--gray); margin-bottom:15px; font-weight:800; padding-left:5px;">나의
                    캠핑 기록</p>

                <div v-for="h in history" :key="h.roomId" :class="['history-item', {active: h.active}]"
                ...
                <div class="message-area" id="msgArea">
                    ...
                    <div v-for="(msg, idx) in messages" :key="idx" :class="['msg', msg.role]">
                        <div v-if="msg.role === 'bot'" class="bot-avatar"
                            style="width:32px; height:32px; font-size:16px;">🔥</div>
            ...
            createApp({
                data() {
                    ...
                    return {
                        ...
                        showFaq: true,
                        messages: [
                            { role: 'bot', message: '반갑닥! 오늘 캠핑 고민은 뭐냐닥?', time: this.fnGetCurrentTime() }
                        ],
                        history: [],
                        recommends: [],
                        ...
                        $.ajax({
                            ...
                            success: (res) => {
                                ...
                                if (this.initialRoomId) {
                                    const target = this.history.find(item => item.roomId == this.initialRoomId);
                                    if (target) {
                                        this.fnSelectHistory(target);
                                    }
                                    this.initialRoomId = '';
                                }
                            }
