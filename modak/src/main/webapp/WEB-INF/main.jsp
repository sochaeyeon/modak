      ...
      <%@ include file="/WEB-INF/common/header.jsp" %>
          ...
          <div class="product-grid">
            <div class="product-card fade-up" onclick="fnGoDetail(1)">
              ...
              <div class="product-info">
                ...
                <div class="product-stars"><span class="star">★★★★★</span><span class="product-reviews">(128)</span>
                </div>
                <div class="product-price"><span class="price-main">35,000</span><span
                    class="price-unit">원/1박</span><span class="price-sale">45,000원</span></div>
            ...
            <div class="product-card fade-up" onclick="fnGoDetail(2)">
              ...
              <div class="product-info">
                ...
                <div class="product-stars"><span class="star">★★★★★</span><span class="product-reviews">(256)</span>
                </div>
                <div class="product-price"><span class="price-main">3,000</span><span class="price-unit">원/1박</span>
                </div>
                <div class="product-btns">
            ...
            <div class="product-card fade-up" onclick="fnGoDetail(3)">
              ...
              <div class="product-info">
                ...
                <div class="product-stars"><span class="star">★★★★☆</span><span class="product-reviews">(74)</span>
                </div>
                <div class="product-price"><span class="price-main">12,000</span><span class="price-unit">원/1박</span>
                </div>
                <div class="product-btns">
            ...
            <div class="product-card fade-up" onclick="fnGoDetail(4)">
              ...
              <div class="product-info">
                ...
                <div class="product-stars"><span class="star">★★★★★</span><span class="product-reviews">(41)</span>
                </div>
                <div class="product-price"><span class="price-main">18,000</span><span class="price-unit">원/1박</span>
                </div>
                <div class="product-btns">
        ...
        <%@ include file="/WEB-INF/common/footer.jsp" %>
          ...
          <script>
              ...
              methods: {
                ...
                loadWeather: async function (region) {
                  ...
                  try {
                    ...
                    if (sR.result === 'success' && sR.data.response.body.items) { // 데이터 존재 확인 추가
                      var items = sR.data.response.body.items.item;
                      var today = this.getDateStr(0), tmr = this.getDateStr(1), byDate = {};

                      items.forEach(function (item) {
                        if (!byDate[item.fcstDate]) byDate[item.fcstDate] = { temps: [], skies: [], ptys: [], rains: [] };
                        var d = byDate[item.fcstDate];

                        if (item.category === 'TMX') d.max = item.fcstValue;
                        if (item.category === 'TMN') d.min = item.fcstValue;

                        // 최고/최저 기온이 없는 시간대 대비를 위해 모든 예보 기온을 수집
                        if (item.category === 'TMP') d.temps.push(parseFloat(item.fcstValue));

                        if (item.category === 'SKY') d.skies.push(item.fcstValue);
                        if (item.category === 'PTY') d.ptys.push(item.fcstValue);
                        if (item.category === 'POP') d.rains.push(item.fcstValue);
                      });

                      [today, tmr].forEach(function (ds, i) {
                        var d = byDate[ds];
                        if (!d) return; // 해당 날짜 데이터가 아예 없으면 건너뜀

                        // 핵심: TMX/TMN이 null이면 temps 배열에서 찾기
                        var mx = (d.max != null) ? Math.round(d.max) : (d.temps.length > 0 ? Math.max(...d.temps) : null);
                        var mn = (d.min != null) ? Math.round(d.min) : (d.temps.length > 0 ? Math.min(...d.temps) : null);

                        // 아이콘 결정 로직 보강
                        var sky = d.skies.length > 0 ? d.skies[Math.floor(d.skies.length / 2)] : '1';
                        var pty = d.ptys.length > 0 ? d.ptys[Math.floor(d.ptys.length / 2)] : '0';
                        var rain = d.rains.length > 0 ? Math.max(...d.rains) : '0'; // 강수확률은 가장 높은 값 사용

                        result.push({
                          label: i === 0 ? '오늘' : '내일',
                          type: '단기',
                          icon: self.skyToIcon(sky, pty),
                          max: mx != null ? mx + '°' : '-°',
                          min: mn != null ? mn + '°' : '-°',
                          rain: rain + '%'
                        });
                    ...
                    // 중기 예보 (기존 로직 유지)
                    if (mR.result === 'success' && mR.ta.response.body.items) {
                      var taItem = mR.ta.response.body.items.item;
                      var ta = Array.isArray(taItem) ? taItem[0] : taItem;
                      var landItem = mR.land.response.body.items.item;
                      var land = Array.isArray(landItem) ? landItem[0] : landItem;
            ...
            function fnGoDetail(no) { var items = JSON.parse(localStorage.getItem('recentViewed') || '[]'); items = items.filter(function (i) { return i.no !== no; }); items.unshift(PM[no]); if (items.length > 10) items.pop(); localStorage.setItem('recentViewed', JSON.stringify(items)); location.href = '/product/product-detail.do?no=' + no; }
            (function () { var items = JSON.parse(localStorage.getItem('recentViewed') || '[]'); if (!items.length) return; var html = ''; items.slice(0, 10).forEach(function (item) { html += '<div class="recent-item" onclick="location.href=\'/product/product-detail.do?no=' + item.no + '\'">' + '<span>' + item.icon + '</span>' + '<span style="font-size:12px;color:var(--brown2)">' + item.name + '</span></div>'; }); document.getElementById('recentItems').innerHTML = html; setTimeout(function () { document.getElementById('recentBar').classList.add('visible'); }, 3000); })();
