...
<div id="loginModal"
  ...
  <div style="background:#fffdf8;border-radius:24px;padding:36px 32px 28px;width:320px;box-shadow:0 24px 64px rgba(44,30,15,.25);text-align:center;animation:modalPop .25s ease;font-family:var(--font-body,'Noto Sans KR',sans-serif);position:relative;z-index:100000;">
    <div style="font-size:48px;margin-bottom:16px">🔐</div>
    <p style="font-size:18px;font-weight:800;color:#2C1E0F;margin-bottom:8px;font-family:var(--font-title,'GgiBatang',serif)">로그인이 필요해요</p>
    <p style="font-size:13px;color:#8B6B4A;line-height:1.7;margin-bottom:24px;font-family:var(--font-body,'Noto Sans KR',sans-serif)">찜 기능은 로그인 후 이용할 수 있어요.<br>로그인 페이지로 이동할까요?</p>
    <div style="display:flex;gap:10px;">
      ...
      <button onclick="location.href='/user/login.do'"
        ...
        onmouseover="this.style.background='#C4621E'" onmouseout="this.style.background='#E8732A'">로그인하기</button>
    </div>
  </div>
</div>
<style>
...
/* ── ★ 3. 페이지 진입 시 배너 아래(히어로 본문)로 즉시 스크롤 ── */
function goHeroContent() {
  var banner = document.getElementById('heroBannerWrap');
  if (banner) {
    window.scrollTo({ top: banner.offsetHeight, behavior: 'instant' });
  }
}
document.addEventListener('DOMContentLoaded', goHeroContent);
window.addEventListener('load', goHeroContent);
...
(function(){
  function renderPopular(list) {
    ...
    grid.addEventListener('click', function(e) {
      ...
      else if (rent) { e.stopPropagation(); location.href = '/product/detail.do?productId=' + rent.dataset.pid; }
      else if (buy)  { e.stopPropagation(); location.href = '/product/detail.do?productId=' + buy.dataset.pid; }
          ...
          success: function(wRes) {
            if (wRes.result === 'success' && wRes.list && wRes.list.length) {
              ...
              document.querySelectorAll('.wish-btn').forEach(function(btn) {
                if (wishedIds.indexOf(parseInt(btn.dataset.pid)) !== -1) btn.classList.add('on');
              });
            }
          }
        });
...
/* ── 11. 카테고리 ── */
(function(){
  var iconMap = { 4:'⛺', 5:'🏕️', 6:'🪑', 7:'🛏️', 8:'🔥', 9:'🍳', 10:'💡', 11:'🔒', 12:'🎒' };
...
(function(){
  ...
  function buildSlides(list) {
    ...
    list.forEach(function(ev, i) {
      ...
      slideHtml +=
        '<div class="heb-slide">' +
        '<div class="heb-card ' + theme + ' heb-card-link" style="cursor:pointer" data-eid="' + eid + '">' +
    ...
    track.addEventListener('click', function(e) {
      ...
      if (btn) { e.stopPropagation(); fnGoBanner(btn.dataset.eid); }
      else if (card) { fnGoBanner(card.dataset.eid); }
    });
    initSlider();
  }
