<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
                ...
                <div class="signup-box" v-if="!signupSuccessed">
                    ...
                    <div class="field">
                        ...
                        <div class="input-wrap input-with-btn">
                            <input v-model="userId" type="text" id="userIdInput" placeholder="아이디를 입력해주세요">
                    ...
                    <div class="field">
                        ...
                        <div class="email-row">
                            <div class="input-wrap email-id-wrap">
                                <input v-model="emailId" type="text" id="emailIdInput" placeholder="이메일"
                                    @input="fnValidateEmailParts">
                            </div>

                            <span class="email-at">@</span>

                            <div class="input-wrap email-domain-wrap">
                                <input v-model="emailDomain" type="text" id="emailDomainInput" placeholder="직접 입력"
                                    :readonly="emailDomainSelect !== 'direct'" @input="fnValidateEmailParts">
                            </div>

                            <select class="email-domain-select" v-model="emailDomainSelect"
                                @change="fnChangeEmailDomain">
    ...
    <script>
        ...
        const app = Vue.createApp({
            ...
            methods: {
                fnChangeEmailDomain() {
                    if (this.emailDomainSelect === 'direct') {
                        this.emailDomain = '';
                        this.$nextTick(() => {
                        ...
                        this.emailDomain = this.emailDomainSelect;
                    }

                    this.fnValidateEmailParts();
                },

                fnBuildEmail() {
                    const id = this.emailId.trim();
                    const domain = this.emailDomain.trim();
                ...
                fnValidateEmailParts() {
                    const email = this.fnBuildEmail();
                    const hint = document.getElementById('emailHint');
                    const idInput = document.getElementById('emailIdInput');
                    const domainInput = document.getElementById('emailDomainInput');

                    if (!hint || !idInput || !domainInput) {
                        return false;
                    }

                    if (!this.emailId.trim() && !this.emailDomain.trim()) {
                    ...
                    idInput.classList.toggle('valid', ok);
                    idInput.classList.toggle('invalid', !ok);
                    domainInput.classList.toggle('valid', ok);
                    domainInput.classList.toggle('invalid', !ok);

                    hint.className = 'field-hint ' + (ok ? 'ok' : 'error');
                    hint.textContent = ok ? '올바른 이메일 형식이에요.' : '이메일 형식을 확인해주세요.';
                ...
                fnSignUp() {
                    ...
                    if (!self.emailId.trim()) {
                        const el = document.getElementById("emailIdInput");
                        const hint = document.getElementById("emailHint");

                        el.classList.add("invalid");
                        hint.className = "field-hint error";
                        hint.textContent = "이메일 아이디를 입력해주세요.";

                        el.focus();
                        showToast("이메일 아이디를 입력해주세요.");
                        return;
                    }

                    if (!self.emailDomain.trim()) {
                        const el = document.getElementById("emailDomainInput");
                        const hint = document.getElementById("emailHint");

                        el.classList.add("invalid");
                        hint.className = "field-hint error";
                        hint.textContent = "이메일 도메인을 입력해주세요.";

                        el.focus();
                        showToast("이메일 도메인을 입력해주세요.");
                        return;
                    }

                    const fullEmail = self.fnBuildEmail();

                    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                    if (!emailRegex.test(fullEmail)) {
                        const idEl = document.getElementById("emailIdInput");
                        const domainEl = document.getElementById("emailDomainInput");
                        const hint = document.getElementById("emailHint");

                        idEl.classList.add("invalid");
                        domainEl.classList.add("invalid");
                        hint.className = "field-hint error";
